...
export const PropertiesPanel: React.FC = () => {
  ...
  // Show panel if a shape is selected OR if a drawing tool is active
  ...
  const hasFill = ['rectangle', 'circle', 'triangle', 'diamond', 'star', 'rect', 'ellipse', 'polygon'].includes(shapeType);
    ...
    <div className={`fixed top-1/2 right-6 transform -translate-y-1/2 p-4 rounded-xl shadow-lg border backdrop-blur-md w-64 max-h-[80vh] overflow-y-auto custom-scrollbar z-20 ${
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold text-xs uppercase tracking-wider text-gray-400">
          {isShapeSelected ? 'Selected Shape' : 'Shape Properties'}
        </h3>
        {isShapeSelected && (
